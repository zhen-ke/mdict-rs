    ...
    map(
        ...
        move |(enc, checksum, encrypted)| {
            ...
            let data: Vec<u8> = match enc_method {
                ...
                _ => {
                    // Log error and return empty vec to avoid panic
                    // Ideally this parser should return Result, but map limits us.
                    // For now, returning empty data is safer than crashing.
            ...
            match comp_method {
                0 => data,
                1 => {
                    let lzo = minilzo_rs::LZO::init().unwrap(); // LZO init rarely fails
                    match lzo.decompress(&data[..], dsize) {
                        Ok(v) => v,
                        Err(e) => {
