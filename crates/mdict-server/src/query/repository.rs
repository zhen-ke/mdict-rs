...
fn query_record_location(
    ...
    // 关键热路径：P0 — 用 `prepare_cached` 而不是 `prepare`。三条 const SQL
    //（EXACT/NOCASE/NORMALIZED）在每个连接上首次调用时各 prepare 一次后入缓存，
    // 之后同一 r2d2 连接复用即可跳过解析/规划轮次。查询是高频点查，单连接级
    // 语句缓存是零行为变化的纯收益（相对每次 `prepare` 重新 tokenize+规划）。
    let mut stmt = conn
        .prepare_cached(sql)
